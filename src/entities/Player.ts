...
export class Player extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    this.backArm = scene.add.rectangle(-9, -4, 7, 24, THEME.colors.playerUniform, 1).setOrigin(0.5, 0.12).setStrokeStyle(2, 0x050505, 0.9);
    ...
    this.frontArm = scene.add.rectangle(12, -4, 7, 24, THEME.colors.playerUniform, 1).setOrigin(0.5, 0.12).setStrokeStyle(2, 0x050505, 0.9);
    ...
    this.hair = scene.add.rectangle(4, -26, 19, 7, THEME.colors.playerHair, 1).setStrokeStyle(1, 0x050505, 1);
    this.mohawk = scene.add.rectangle(14, -29, 18, 8, THEME.colors.playerHair, 1).setStrokeStyle(1, 0x050505, 1);
  ...
  private updateSidePose(state: 'idle' | 'walk' | 'dash'): void {
    ...
    this.mohawk.setRotation(state === 'dash' ? -0.08 + lean * 0.2 : -0.04 + lean * 0.15);
    ...
    this.hair.setY(state === 'dash' ? -25 : -26 + phase * 0.2);
    this.mohawk.setPosition(state === 'dash' ? 15 : 14, state === 'dash' ? -28 : -29 + phase * 0.1);
    ...
    const shoulderY = state === 'dash' ? -3 : -4;
    this.frontArm.setPosition(12 - phase * arm * 0.18, shoulderY);
    this.backArm.setPosition(-9 + phase * arm * 0.14, shoulderY + 1);
    this.frontArm.setRotation(-0.08 - phase * (state === 'dash' ? 0.5 : 0.2));
    this.backArm.setRotation(0.1 + phase * (state === 'dash' ? 0.46 : 0.18));
